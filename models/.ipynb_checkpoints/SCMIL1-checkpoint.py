...
class HierarchicalClusterLocalAttention(nn.Module):
    def __init__(self, hidden_size=384, n_cluster=None, cluster_size=None, feature_weight=0, dropout_rate=0.25):
        ...
        self.local_attention = HierarchicalSelfAttention(dim=hidden_size, num_heads=32, dropout_prob=dropout_rate)
        self.global_attention = HierarchicalSelfAttention(dim=hidden_size, num_heads=32, dropout_prob=dropout_rate)
...
class SCMIL(nn.Module):
    def __init__(self, n_classes=4, input_size=1536, hidden_size=None, deep=1, n_cluster=None, cluster_size=256,
                 feature_weight=0, as_backbone=False, dropout_rate=0.25, with_softfilter=False, use_filter_branch=False,
                 with_cssa=True, softfilter_quantile=0.25, **kwargs):
        super().__init__()
        ...
        if self.with_softfilter:
            self.softfilter = SoftFilterLayer(dim=input_size, hidden_size=1024, deep=1, quantile=softfilter_quantile)
        ...
        self.amil = AMIL_layer(hidden_size, 1024, dropout=dropout_rate)
