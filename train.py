...
def train(model, train_loader, val_loader, cfg, save_dir, metircs, with_coords=False):
    ...
    if cfg.optimizer.loss_func == 'egmde_ml':
        loss_func = NLLELGE()
    else:
...
def main(args, cfg, save_dir, fold):
    ...
    if cfg.datasets.type == 'tcga-survival-egmdm-wsi':
        ...
        train_ds = DataGeneratorTCGASurvivalWSIEGMDE(anno_path,train_ids_path,clinical_path,shuffle=True,with_coords=with_coords,cluster_label_path=cluster_label_path)
        val_ds = DataGeneratorTCGASurvivalWSIEGMDE(anno_path,val_ids_path,clinical_path,shuffle=True,with_coords=with_coords,cluster_label_path=cluster_label_path)
    else:
        raise NotImplementedError
    print(
